...
When the registration file also advertises ERC-8004 progressive validation requests, the same audit exports `validationLineage` receipts in sweep JSON entries. Each receipt groups responses by `requestHash` and original validator, verifies `feedbackURI`/`responseURI` content against `feedbackHash`/`responseHash` when present, preserves response order/tags, and marks the latest score/tag state so downstream reputation consumers do not collapse soft and final validator updates into one opaque score. Full-weight lineage also requires a signed event-storage completeness verdict that retains the canonical `NewFeedback` event pointer, reconciles event-only `endpoint`/`feedbackURI`/`feedbackHash` fields with the stored value/tag/revocation snapshot, and exports any missing-field decision before reputation consumers ingest the evidence.
